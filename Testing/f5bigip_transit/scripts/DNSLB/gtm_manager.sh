...
then
     ...
     echo -e "\033[92m|Ex)./gtm_manager.sh modify mode[ratio/datacenter/wideip]       |\033[0m"
...
then
  ...
  then
    ...
    curl -sk -H "X-F5-Auth-Token: $token" -X GET https://$ip/mgmt/tm/gtm/pool/a | jq -r '.items[] | .name + " " + .loadBalancingMode'
    echo -e "\033[32m---------------------------------------\033[0m "
    pname=$(curl -sk -H "X-F5-Auth-Token: $token" -X GET https://$ip/mgmt/tm/gtm/pool/a | jq -r .items[1].name)

    if [ "$3" == "member" ]
    then
      echo -e "\033[32mGetting pool member detail ............. \033[0m "
      curl -sk -H "X-F5-Auth-Token: $token" -X GET https://$ip/mgmt/tm/gtm/pool/a/$pname/members | jq -r .
    elif [ "$3" == "detail" ]
    then
      echo -e "\033[32mGetting pool detai ............. \033[0m "
      curl -sk -H "X-F5-Auth-Token: $token" -X GET https://$ip/mgmt/tm/gtm/pool/a | jq -r .
    else
      pname=$3
      echo -e "\033[32mGetting pool member ............. \033[0m "
